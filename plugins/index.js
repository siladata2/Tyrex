...
try {
  ...
  if (vvMod && typeof vvMod.handleAutoVV === 'function') handleAutoVV = vvMod.handleAutoVV;
} catch { console.warn('⚠️ viewonce module not found.'); }
...
function setupHandlers(conn, number, saveCreds) {
  ...
  conn.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      try { await handleAutoVV(conn, msg); } catch(e){ console.error(`autoVV: ${e.message}`); }
