...
const configPath = path.join(process.cwd(), 'data', 'autoreplies.json');
    ...
    async handler(sock, message, args, context = {}) {
        ...
        try {
            ...
            if (action === 'on' || action === 'enable') {
                if (config.enabled) {
                    return await sock.sendMessage(chatId, { text: '⚠️ *Auto-reply is already enabled*', ...channelInfo }, { quoted: message });
                }
                config.enabled = true;
            ...
            if (action === 'off' || action === 'disable') {
                if (!config.enabled) {
                    return await sock.sendMessage(chatId, { text: '⚠️ *Auto-reply is already disabled*', ...channelInfo }, { quoted: message });
                }
