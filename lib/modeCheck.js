/*****************************************************************************
 *  lib/modeCheck.js — TYREX_KSH MD RC12 ULTRA REWRITE
...
function modeLabel(mode) {
    ...
    const labels = {
        public:  `${SYM.circle} PUBLIC    ${SYM.arrow} All users, all chats`,
        groups:  `${SYM.gem} GROUPS    ${SYM.arrow} Group chats only`,
        inbox:   `${SYM.leaf} INBOX     ${SYM.arrow} DM / private chats only`,
        private: `${SYM.diamond} PRIVATE   ${SYM.arrow} Owner and sudo only`,
        self:    `${SYM.star} SELF      ${SYM.arrow} Owner only (strictest)`,
    };
    return labels[m] || `${SYM.cross} UNKNOWN (${m}) ${SYM.arrow} Treated as public`;
}
