...
const fetchJson = async (url, options = {}) => {
  try {
    const res = await axios({ method: 'GET', url, headers: { 'User-Agent': 'Mozilla/5.0 TYREX_KSH MD/5.0' }, ...options });
    return res.data;
...
const fetchPostJson = async (url, data, options = {}) => {
  try {
    const res = await axios.post(url, data, { headers: { 'User-Agent': 'TYREX_KSH MD/5.0' }, timeout: 15000, ...options });
    return res.data;
  } catch (err) { console.error(err.message); return null; }
};
