...
const ffmpegStatic = require('ffmpeg-static'); // ✅ use static binary
...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./XeonMedia/trash/${filename}.gif`, image)
					 child_process.exec(
								`"${ffmpegStatic}" -i ./XeonMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./XeonMedia/trash/${filename}.mp4`
										) 
  await sleepy(4000)
