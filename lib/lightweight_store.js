 ...
 *  💬  WhatsApp : https://whatsapp.com/channel/0029VbDAQiXHbFV0iSwCtz2o     *
 *                                                                           *
...
/**
 * ✅ FIX: a backend is only usable while its connection is healthy.
 * (Mongo was activated without verifying the connection before — every
 * store call then hung ~30s on server selection and silently failed,
 * which made the bot feel extremely slow and "lost" saved settings.)
 */
function backendUsable() {
  if (backend === 'memory') return false
  try { return !adapters[backend].isReady || adapters[backend].isReady() } catch { return true }
}
...
if (MONGO_URL) {
  try {
    ...
    // ✅ FIX: a rejected connect() was only logged, never surfaced anywhere
    // else, and there was no listener for errors that happen *after* the
    // initial connect (drops, auth expiry, etc.) — those became silent
    // unhandled rejections. Both are now covered.
    //
    // ✅ FIX: "bad auth : authentication failed" (wrong MONGO_URL user/pass)
    // is NOT a transient network blip — the Mongo driver was retrying server
    // selection on it forever anyway, which on Render's free tier (shared,
    // tiny CPU quota) burned cycles on every single message and was a real
    // contributor to the bot feeling slow, plus it spammed identical error
    // logs nonstop. Bad auth now disconnects immediately instead of
    // retrying — store falls back to file storage cleanly, once, with one
    // clear log line telling you to fix MONGO_URL instead of infinite noise.
    let mongoAuthFailed = false;
    mongoose.connect(MONGO_URL, {
      serverSelectionTimeoutMS: 8000,
      connectTimeoutMS: 10000,
      serverApi: { version: '1', strict: true, deprecationErrors: true }
    }).catch(err => {
      console.error('[MONGO] Connection error:', err?.stack || err?.message || err)
      if (/bad auth|authentication failed/i.test(err?.message || '')) {
        mongoAuthFailed = true
        console.error('[MONGO] ❌ Bad credentials in MONGO_URL — check the username/password (and that the DB user exists / IP is allowlisted in Atlas). NOT retrying — falling back to file storage.')
        mongoose.disconnect().catch(()=>{})
      }
    })

    mongoose.connection.on('error', (err) => {
      if (mongoAuthFailed) return // already handled above, don't spam
      if (/bad auth|authentication failed/i.test(err?.message || '')) {
        mongoAuthFailed = true
        console.error('[MONGO] ❌ Bad credentials in MONGO_URL — check the username/password (and that the DB user exists / IP is allowlisted in Atlas). NOT retrying — falling back to file storage.')
        mongoose.disconnect().catch(()=>{})
      ...
      console.error('[MONGO] Connection error event:', err?.stack || err?.message || err)
    })
    ...
    // ✅ FIX: only switch to the mongo backend AFTER a verified connection.
    // Previously the backend switched immediately, so an unreachable/bad URI
    // made every getSetting/saveSetting hang ~30s then error — slow bot AND
    // lost settings. Now: wait up to ~12s for readyState 1, else stay on
    // memory + JSON files (which also keeps settings working).
    ;(async () => {
      try {
        ...
        if (mongoose.connection.readyState === 1) {
          ...
          printLog('store', 'MongoDB connected ✅ — unlimited storage, full persistence')
        } else {
          printLog('warning', 'MongoDB NOT reachable — using memory + JSON file storage. Check MONGO_URL.')
        }
      } catch (e) {
        // ✅ FIX: this whole wait-loop used to run unguarded inside an async
        // IIFE — a throw in here became an unhandled promise rejection
        // instead of the clean "warning" log below, which is exactly what
        // produced the garbled "(intermediate value)... is not a function"
        // console output (V8's generic formatting for an error thrown deep
        // in an un-awaited chain). Now it's caught locally with the full
        // stack logged, and the bot still falls back to memory/JSON safely.
        console.error('[MONGO] init wait-loop error:', e?.stack || e?.message || e)
        printLog('warning', `MongoDB init failed during connection wait: ${e?.message || e}`)
      }
    })()
  } catch (e) {
    console.error('[MONGO] init error (full):', e?.stack || e)
    printLog('warning', `MongoDB initialization failed: ${e?.message || e}`)
  }
}
...
const store = {
  ...
  async saveSetting(chatId, key, value) {
    if (backend === 'memory' || !backendUsable()) {
      // ✅ FIX: mirror settings to Supabase too — durable across restarts even
      // when the file system is ephemeral (Render/Railway free tiers).
      try {
        const supabaseStore = require('./supabaseStore')
        if (supabaseStore.isEnabled()) await supabaseStore.saveSetting(chatId, key, value)
      } catch {}
      const dataDir = './data'
  ...
  async getSetting(chatId, key) {
    if (backend === 'memory' || !backendUsable()) {
      ...
      try {
        ...
        // ✅ FIX: fall back to Supabase — keeps settings after restarts
        try {
          const supabaseStore = require('./supabaseStore')
          if (supabaseStore.isEnabled()) {
            const v = await supabaseStore.getSetting(chatId, key)
  ...
  */
  ...
  async setBotMode(mode) {
    ...
    // Always update global.BOT_MODE so index.js handler reads it correctly
  ...
  async getBotMode() {
    // Always prefer global.BOT_MODE (set by index.js handler) for consistency
    if (typeof global !== 'undefined' && global.BOT_MODE) return global.BOT_MODE;
    if (backend === 'memory') {
...
// ✅ SPEED FIX: getSetting() is called on EVERY message by the per-message
// plugins (antilink, antibadword, antiflood, chatbot…). On the memory/file
// backend it did a synchronous fs.readFileSync per call; on Mongo/PG/MySQL it
// did a network round-trip per call. Multiplied by several plugins × every
// message, this was a major source of slow replies. We wrap getSetting with a
// short-TTL in-memory cache and invalidate it write-through on every save.
const _settingCache = new Map();  // `${chatId}\u0000${key}` -> { val, ts }
const _SETTING_TTL = 8_000;       // 8s
const _sk = (chatId, key) => `${chatId}\u0000${key}`;

const _origGetSetting = store.getSetting.bind(store);
const _origSaveSetting = store.saveSetting.bind(store);

store.getSetting = async function (chatId, key) {
  const k = _sk(chatId, key);
  const c = _settingCache.get(k);
  const now = Date.now();
  if (c && (now - c.ts) < _SETTING_TTL) return c.val;
  const val = await _origGetSetting(chatId, key);
  _settingCache.set(k, { val, ts: now });
  // keep the cache from growing unbounded
  if (_settingCache.size > 5000) {
    const cutoff = now - _SETTING_TTL;
    for (const [mk, mv] of _settingCache) if (mv.ts < cutoff) _settingCache.delete(mk);
  }
  return val;
};

store.saveSetting = async function (chatId, key, value) {
  // update cache immediately so the very next read is correct & instant
  _settingCache.set(_sk(chatId, key), { val: value, ts: Date.now() });
  return _origSaveSetting(chatId, key, value);
};

store.invalidateSettingCache = function (chatId, key) {
  if (chatId === undefined) { _settingCache.clear(); return; }
  if (key === undefined) {
    const prefix = `${chatId}\u0000`;
    for (const mk of _settingCache.keys()) if (mk.startsWith(prefix)) _settingCache.delete(mk);
    return;
  }
  _settingCache.delete(_sk(chatId, key));
};

// ✅ FIX: plugins call store.setSetting() (antiflood, groupguard, security…)
// but only saveSetting() existed — every setSetting call threw a TypeError
// and configs were never persisted. Alias it to saveSetting.
store.setSetting = store.saveSetting;
