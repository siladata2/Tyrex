...
async function loadUserGroupData() {
    const now = Date.now();
    if (_ugdCache && now - _ugdCacheTs < UGD_TTL) return _ugdCache;
    try {
        if (HAS_DB) {
            ...
            if (!fs.existsSync(dataPath)) {
                ...
                _ugdCache = defaultData;
            } else {
                _ugdCache = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
            }
        }
        _ugdCacheTs = now;
        return _ugdCache;
...
async function saveUserGroupData(data) {
    try {
        ...
        // Update cache so next read doesn't go to DB
        _ugdCache = data;
        _ugdCacheTs = Date.now();
        return true;
    } catch (error) {
...
async function isSudo(userId) {
    try {
        const data = await loadUserGroupData();
        if (!data.sudo || !userId) return false;
        if (data.sudo.includes(userId)) return true;
        const norm = (s) => {
        ...
        return data.sudo.some(s => norm(s) === userNorm);
    } catch (error) {
...
async function addWelcome(jid, enabled, message) {
    try {
        ...
        data.welcome[jid] = {
            ...
            channelId: '120363426816577327@newsletter'
        };
