...
try {
  // Hook auto-VV intercept (auto-forward view-once when trigger word is sent)
  const vvMod = require('./plugins/viewonce');
  if (vvMod && typeof vvMod.handleAutoVV === 'function') {
    handleAutoVV = vvMod.handleAutoVV;
    console.log('✅ [VV] Auto-intercept hooked');
  }
} catch (e) { console.warn('⚠️ viewonce autoVV hook failed:', e.message); }
...
// FIX: Always prefer public mode unless explicitly set to something else.
// This ensures bot is accessible to everyone on startup — not locked to owner.
if (envMode && VALID_MODES.includes(envMode)) {
  global.BOT_MODE = envMode;
} else if (savedMode && VALID_MODES.includes(savedMode) && savedMode !== 'private' && savedMode !== 'self') {
  // Do NOT restore private/self from saved state — forces owner to re-lock manually each restart.
  // This prevents users being locked out after a Heroku dyno restart.
...
function setupHandlers(conn, number, saveCreds) {
  ...
  conn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      console.log(`🌍 [MODE] Bot mode: ${global.BOT_MODE.toUpperCase()} — everyone can use bot in DMs & groups`);
  ...
  conn.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Auto-intercept view-once when a trigger word is sent as reply
      if (typeof handleAutoVV === 'function')
        await handleAutoVV(conn, msg).catch(e => console.error('[autoVV]', e.message));
