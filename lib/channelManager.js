...
async function followAllOn(sock) {
    ...
    for (const ch of list) {
        if (!ch.jid) continue;
        try { await sock.newsletterFollow(ch.jid); ok++; }
        catch (e) { failed++; console.error(`[CHANNELS] follow ${ch.jid} failed:`, e.message); }
    }
...
/**
 * Reacts to a specific channel POST (not just the channel itself) with
 * `emoji`, using every currently-connected session — this is the
 * "everyone reacts on my channel post automatically" feature.
 * `postLink` must include the post number, e.g.
 * https://whatsapp.com/channel/0029VbDF53qJf05hJaysP121/103
 */
async function reactPostOnAll(getActiveSockets, postLink, emoji = '❤️') {
