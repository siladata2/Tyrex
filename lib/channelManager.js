...
async function followAllOn(sock) {
    const list = await loadChannels();
    let ok = 0, failed = 0;
    // ✅ FIX ("all follow but one failed"): WhatsApp rate-limits rapid
    // newsletterFollow calls, so the last one(s) in a burst silently fail.
    // Space each follow out and retry once after a short backoff. This makes
    // every saved channel get followed reliably instead of dropping one.
    for (let idx = 0; idx < list.length; idx++) {
        ...
        for (let attempt = 1; attempt <= 2 && !done; attempt++) {
            try {
                await sock.newsletterFollow(ch.jid);
                ok++; done = true;
            } catch (e) {
                if (attempt === 2) {
                    failed++;
                    console.error(`[CHANNELS] follow ${ch.jid} failed:`, e.message);
                } else {
                    await wait(2500); // backoff before retry
                }
            }
        }
